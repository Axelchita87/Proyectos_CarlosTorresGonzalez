...
-- =============================================
CREATE Procedure [dbo].[GetAndSetAttribute1]
 @Location VARCHAR(6),
 @OrgId int
As
...
--Select @LocationIntegra=LOCATION,@ATTRIBUTE1=ATTRIBUTE1,@Status=STATUS,@Status_Site=STATUS_SITE 
--From CORPP..APPS.CTS_CLIENTES_V Where LOCATION='MAB115' --And (@Status_Site='A' And @Status='A')
--SELECT @LocationIntegra,@ATTRIBUTE1,@Status,@Status_Site 
If Exists(Select LOCATION From CTS_CLIENTES_V Where LOCATION=@Location AND ORG_ID=@OrgId)
Begin
  Select @ATTRIBUTE1=ATTRIBUTE1,@Status=[STATUS],@Status_Site=STATUS_SITE 
  From CTS_CLIENTES_V Where LOCATION=@Location And (Status_Site='A' And [Status]='A') AND ORG_ID=@OrgId
