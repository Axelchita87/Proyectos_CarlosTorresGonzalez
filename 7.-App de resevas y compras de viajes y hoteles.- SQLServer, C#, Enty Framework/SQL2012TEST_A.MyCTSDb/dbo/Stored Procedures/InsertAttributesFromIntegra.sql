...
-- Create date: <May 11, 2011>
-- Description:	<Procedure that compares the view CTS_CLIENTES_V and the table dbo.Attributes, fills dbo.Attributes and dbo.QualityControls tables>
-- =============================================
CREATE PROCEDURE [dbo].[InsertAttributesFromIntegra]
AS
BEGIN
DECLARE @Attribute1 AS VARCHAR(6)
DECLARE @OrgId AS INT

DECLARE @CompareAttribute1 AS VARCHAR(6)

DECLARE Attributes_cursor CURSOR FAST_FORWARD FOR
SELECT DISTINCT ATTRIBUTE1, ORG_ID FROM CORPP..APPS.CTS_CLIENTES_V WHERE STATUS='A' AND STATUS_SITE='A'

OPEN Attributes_cursor
	
	FETCH next FROM Attributes_cursor INTO
	@Attribute1, @OrgId
		
		WHILE @@FETCH_STATUS = 0
		BEGIN
			
			SELECT @CompareAttribute1=Attribute1 FROM dbo.Attributes
			WHERE Attribute1=@Attribute1
			
			IF (@Attribute1=@CompareAttribute1)
			BEGIN
				
				UPDATE dbo.Attributes 
				SET OrgId=@OrgId
				WHERE Attribute1=@CompareAttribute1
			END

			IF (@CompareAttribute1='')
			BEGIN
				
				INSERT INTO  dbo.Attributes (Attribute1, NameMyCTS, StatID, Reservation, Emision, OrgId)
				VALUES (@Attribute1, 'SIN_CORP', 1, 1, 1, @OrgId)
