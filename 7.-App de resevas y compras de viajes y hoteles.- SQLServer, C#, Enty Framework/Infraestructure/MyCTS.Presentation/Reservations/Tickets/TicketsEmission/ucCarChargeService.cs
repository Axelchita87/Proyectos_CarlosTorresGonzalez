...
{
    public partial class ucCarChargeService : CustomUserControl
    {
        ...
        private void ucHotelChargeService_Load(object sender, EventArgs e)
        {
            ...
            if (profile.CreditCards.HasCards)
            {
                using (var form = new MyCTS.Presentation.Reservations.Availability.InterJet.frmInterJetProfileCreditCards())
                {
                    ...
                    //SetSel¿aectedCreditCardFromProfile(form.SelectedCreditCardCard);
                }
            }
        }
        ...
        private void ValidacionFormasDePago()
        {
            ...
            {
                MessageBox.Show("Ya cuenta con un intento de cargo por servicio previo para Autos, no es posible realizar un nuevo cargo por servicio para Autos mediante la interfaz", Resources.Constants.MYCTS, MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
        }
        ...
        {
            CxSHoteles datosTran = GetCxSHotelesBL.GetCxSHoteles(string.Concat(recLoc, "|CAR"));
            if (datosTran != null)
            {
        ...
        private void DeterminaValidacionGeneracionCargo()
        {
            ...
            ucServicesFeePayApply.OrigenTipo = ChargesPerService.OrigenTipoCargo.Autos;

            if (!IsValidBussinesRules)
            {
                bool erase = true;

                foreach (var lstDatosTarjeta2 in ucServicesFeePayApply.lstDatosTarjeta == null ? new List<CreditCardInfo>() : ucServicesFeePayApply.lstDatosTarjeta )
                {
                    if (lstDatosTarjeta2.OrigenVenta == "H")
                    {
                        erase = false;
                        break;
                    }
