function [ensemble] = GA_4_EnsembleRawFit(ensemble, Network, type, var, sets)
...
[Population, Offspring, sortedPop, vars_per_gen, selected] = setUpVarsRawFit(sizePop, ...
...
while(1)
    ...
    Population.fitness = eval_fitnessRawFit(Population,sizeInd,sizePop, ...
...
if(strcmp(type,'RawFit'))             %If the GA is for RBLC
    ...
    %PreF_norm
    [ensemble.Pred, ensemble.weights] = GA_calcWeightsRawFit(ensemble.Pred, ...
        ensemble.weights, cont, NetworkEns, texttoPredict);
