...
for Sig = 0.1:0.1:2
    for i = poshidden
        RES2(p,i) = localConnSnoNorm(5,i,Sig);
    end
    p = p + 1;
end
...
for i= posinputs
    for j = poshidden
        RES(i,j) = localConnSnoNorm(i,j,2);
    end
end
...
if noInp > noHid
    ...
    for Sig = 0.1:0.1:1
        for i = posinputs
            RES1a(p,i) = localConnSnoNorm(1,i,Sig);
        end
        p = p + 1;
    end
    ...
    for Sig = 0.1:0.1:1
        for i = posinputs
            RES2a(p,i) = localConnSnoNorm(5,i,Sig);
        end
        p = p + 1;
    end
    ...
    for i= poshidden
        for j = posinputs
            RESa(j,i) = localConnSnoNorm(j,i,1);
        end
    end
