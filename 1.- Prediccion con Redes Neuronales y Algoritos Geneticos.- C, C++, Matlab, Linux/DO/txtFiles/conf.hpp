...
// Kind of prediction
int kindPred = MSP;						// ( MSP , SSP)    for classification always use SSP cause predI is setup for SSP
static const int DeltaT = 4;								// This is the step size to predict, it is independent of the MSP or SSP
int Pred_stepAhead = 21;			// How many steps to predict / Classify in the final data set. This values should be bigger than Pre_stepAheadInside, if not there is an error in predParam in the initialization or manipulation
...
int sizeVali = 100;										// determinate the number of values to use as validation in case it is not set automatically,
...
int maxdata = 4500;														// all the information used, training + validation + test (for the outside set, i.e. after the evolution finish)
...
static const int totalMaxInput = 30;				// so I have a maximum of 49 inputs 50 hidden and 1 output = a matrix of 100x100 of fixed size
																				// to optimise for Classification use the number of inputs here
static const int totalMaxHidden = 50;
