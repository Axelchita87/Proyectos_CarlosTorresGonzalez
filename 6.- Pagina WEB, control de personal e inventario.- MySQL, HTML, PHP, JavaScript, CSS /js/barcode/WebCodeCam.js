...
(function($, window, document, undefined) {
	...
	var Self, lastImageSrc, con, display, w, h,
		...
		flipped = false,
		isStreaming = false,
		DecodeWorker = new Worker("js/barcode/DecoderWorker.js"),
		delay = false,
	...
	Plugin.prototype = {
		...
		checkOrientation: function(){
 			switch(window.orientation){
           		...
           		case 0:
           			Self.options.currMode = "portrait";
           			break;
 
           		case -90:
           			Self.options.currMode = "landscape";
           			break;
     		...
     		console.log("Orientacion1: "+window.orientation);
		...
		setEventListeners: function() {
			camera.addEventListener('canplay', function(e) {
				if (!isStreaming) {
					...
					console.log("Orientation: "+Self.options.currMode);
					if (Self.options.currMode=='portrait'){
						display[0].setAttribute('width', w);
						display[0].setAttribute('height', h);
					}else{
						display[0].setAttribute('width', h);
						display[0].setAttribute('height', w);
					}
