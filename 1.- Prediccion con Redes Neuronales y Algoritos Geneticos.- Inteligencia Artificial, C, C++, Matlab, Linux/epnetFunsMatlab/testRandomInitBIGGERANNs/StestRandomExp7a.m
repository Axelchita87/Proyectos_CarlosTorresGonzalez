...
% StestRandomExp7a.m
%
% Here is generated randomly the networks using an exponential function
% using the non-normalized distance between nodes, here all lines are fill
% first before it is change the column (fill be lines, creo)
% All sub-matrices are filled the same, except IO and OO, I will not use 
% them here
% 
% To run the first part ofthis exp put minOut and maxOut to 1, 
% in the second part put maxOut to n
%
% plot with exp plotExp1y5Both('resExp7');
%
% Created:      15 Nov 2010
% Modified at:  
...
%             idxOutput = 1:noOut;
            ...
            %IH
            for i = hiddenN
                probMat = fillLinesEnonN(probMat,i,posinputs,sigmaD);
            end
            
            % I can run another exp not usign this
            %IO
%             for i = posinputs
%                 probMat = fillColsEnonN(probMat,i,outputN,sigmaD);
%             end
                                       
            %HH
            idxi = 1;
            for i = hiddenN( 1,2:(size(hiddenN,2)) )
                probMat = fillLinesEnonN(probMat,i,hiddenN(1,1:idxi), sigmaD);
                idxi = idxi + 1;
            end
            
             %HO
             for i = outputN
                probMat = fillLinesEnonN(probMat,i,hiddenN,sigmaD);
             end
