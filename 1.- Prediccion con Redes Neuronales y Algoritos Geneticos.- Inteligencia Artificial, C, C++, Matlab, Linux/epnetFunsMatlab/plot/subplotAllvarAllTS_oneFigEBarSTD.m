...
for TSdir = [1 2 3 5]%1:size(TS,2)         %for all TS
   ...
   hold all
    ...
    if info{1,1}.flagModule1 == 1
        ...
        hold all
        for alldir = 1:sizeDir
           h = errorbar(info{1,alldir}.avMweight,info{1,alldir}.stdMweight,'LineWidth',1);
            %text(150,.1,'\leftarrow Strict value');
        end
        ...
        hold all
        for alldir = 1:sizeDir
             h = errorbar(info{1,alldir}.avMarch,info{1,alldir}.stdMarch,'LineWidth',1);
            %text(150,.1,'\leftarrow Strict value');
        end
        ...
        hold all
        for alldir = 1:sizeDir
           h = errorbar(info{1,alldir}.avIsolated, info{1,alldir}.stdIsolated, 'LineWidth',1);
            %text(150,.1,'\leftarrow Strict value');
        end
