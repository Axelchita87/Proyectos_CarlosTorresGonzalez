...
for TSdir = [1 2 3 5]%1:size(TS,2)         %for all TS
    ...
    for directory=1:sizeDir %for all exp.
        ...
        averagelRate{1,directory} = zeros(1,generation);
        
        avMweight{1,directory} = zeros(1,generation);
        avMarch{1,directory} = zeros(1,generation);
        avNoShared{1,directory} = zeros(1,generation);
        avIsolated{1,directory} = zeros(1,generation);
        
        
        if ( allrun{1,1}.var.task2solve ==  allrun{1,1}.C.CLASSIFY )
            averageClassifE{1,directory} = zeros(1,generation);
        end
        ...
        for i=1:corrida
            ...
            averageNRMS{1,directory} = averageNRMS{1,directory} + allrun{1,i}.ALLParam.Avfitness;
            ...
            averagelRate{1,directory} = averagelRate{1,directory} + allrun{1,i}.ALLParam.Avlrate;
            
            avMweight{1,directory} = avMweight{1,directory} + allrun{1,i}.ALLParam.AvMweight;
            avMarch{1,directory} = avMarch{1,directory} + allrun{1,i}.ALLParam.AvMarch;
            avNoShared{1,directory} = avNoShared{1,directory} + allrun{1,i}.ALLParam.AvNoSharedNodes;
            avIsolated{1,directory} = avIsolated{1,directory} + allrun{1,i}.ALLParam.AvIsolatedNodes;
            
            if ( allrun{1,1}.var.task2solve ==  allrun{1,1}.C.CLASSIFY )
                averageClassifE{1,directory} = averageClassifE{1,directory} + allrun{1,i}.ALLParam.AvClassifError;
            end
        ...
        averagelRate{1,directory} = averagelRate{1,directory}/corrida;
        
        avMweight{1,directory} = avMweight{1,directory} /corrida;
        avMarch{1,directory} = avMarch{1,directory} /corrida;
        avNoShared{1,directory} = avNoShared{1,directory} /corrida;
        avIsolated{1,directory} = avIsolated{1,directory}/corrida;
        
        
        if ( allrun{1,1}.var.task2solve ==  allrun{1,1}.C.CLASSIFY )
            averageClassifE{1,directory} = averageClassifE{1,directory}/corrida;
        end
