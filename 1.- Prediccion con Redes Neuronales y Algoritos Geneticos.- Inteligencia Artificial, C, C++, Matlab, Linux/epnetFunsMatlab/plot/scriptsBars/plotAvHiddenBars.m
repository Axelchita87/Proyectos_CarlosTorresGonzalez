...
if  size( infoBar.avFitness, 2 ) > 1
    ...
    if strcmp(typePlot, 'AV')
        hold off;
        h = bar(xVals, infoBar.avHid);
        set(gca,'XTickLabel',xTicksL);
    elseif strcmp(typePlot, 'STD')
        h = errorb(infoBar.avHid,infoBar.stdHid,'top');
        set(gca,'XTickLabel',xTicks);
    elseif strcmp(typePlot, 'STE')
        h = errorb(infoBar.avHid,infoBar.steHid,'top');
        set(gca,'XTickLabel',xTicks);
    end
    ...
    if strcmp(typePlot, 'AV')
        hold off;
        bar(xVals, infoBar.avHid');
        set(gca,'XTickLabel',xTicksL);
    elseif strcmp(typePlot, 'STD')
        bar(xVals, infoBar.avHid');
        errorb(infoBar.avHid',infoBar.stdHid','top');
        set(gca,'XTickLabel',xTicks);
    elseif strcmp(typePlot, 'STE')
        bar(xVals, infoBar.avHid');
        errorb(infoBar.avHid',infoBar.steHid','top');
        set(gca,'XTickLabel',xTicks);
    end
