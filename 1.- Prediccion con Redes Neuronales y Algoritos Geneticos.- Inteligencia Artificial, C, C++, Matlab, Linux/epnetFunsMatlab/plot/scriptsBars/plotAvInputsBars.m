...
if  size( infoBar.avFitness, 2 ) > 1
    ...
    if strcmp(typePlot, 'AV')
        hold off;
        h = bar(xVals, infoBar.avInp);
        set(gca,'XTickLabel',xTicksL);
    elseif strcmp(typePlot, 'STD')
        h = errorb(infoBar.avInp,infoBar.stdInp,'top');
        set(gca,'XTickLabel',xTicks);
    elseif strcmp(typePlot, 'STE')
        h = errorb(infoBar.avInp,infoBar.steInp,'top');
        set(gca,'XTickLabel',xTicks);
    end
    ...
    if strcmp(typePlot, 'AV')
        hold off;
        bar(xVals, infoBar.avInp');
        set(gca,'XTickLabel',xTicksL);
    elseif strcmp(typePlot, 'STD')
        bar(xVals, infoBar.avInp');
        errorb(infoBar.avInp',infoBar.stdInp','top');
        set(gca,'XTickLabel',xTicks);
    elseif strcmp(typePlot, 'STE')
        bar(xVals, infoBar.avInp');
        errorb(infoBar.avInp',infoBar.steInp','top');
        set(gca,'XTickLabel',xTicks);
    end
end
