 ...
 *
 * This file is dedicate for the functions for the swap connection, in fact the general algorithm it is the same, there is no difference to hierarchical
 * 
 */

#ifndef C_EPNETSWAP_CONN_HPP
#define C_EPNETSWAP_CONN_HPP


class C_epnetSWAP_CONN: public C_EPNet{
public:
	// methods

	//Constructor and destructor
	C_epnetSWAP_CONN( char fileRun[] ) :  C_EPNet( fileRun ){
...
void C_epnetSWAP_CONN::startEPNet(void){
	/*!
	...
	try{
		...
		while(Generation < expectedGenerations){
			...
			if(Ppopulation[selected]->status[0] == SUCCESS){
			...
			// Train with Simulate Annealing (SA) ///////////////////////
				...
				if( ( (net1->fitness[0]*100) / Ppopulation[selected]->fitness[0])  <= STP   &&   net1->fitness[0] > 0   &&   useSA == ON){
